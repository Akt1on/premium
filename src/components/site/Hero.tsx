...
export function Hero() {
  ...
  const bg = useMedia("hero_image", heroImg);
  ...
  return (
    <section ref={ref} id="top" className="relative h-screen min-h-[760px] w-full overflow-hidden">
      ...
      >
        ...
        <h1 className="font-display leading-[0.9] tracking-tight max-w-[1400px]"
            style={{ fontSize: "clamp(2.4rem, 9vw, 9rem)", letterSpacing: "0" }}>
          {["СТРОИМ ДОРОГИ.", "СТРОИМ РЕПУТАЦИЮ."].map((line, i) => (
            <motion.span
              key={line}
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            >
              <span className="relative z-10">Рассчитать объект</span>
      ...
      {/* Rotating badge */}
      <div className="hidden md:flex absolute right-8 lg:right-16 top-1/2 -translate-y-1/2 z-10 w-36 h-36 lg:w-44 lg:h-44 items-center justify-center">
        <div className="absolute inset-0 animate-spin-slow">
          <svg viewBox="0 0 200 200" className="w-full h-full">
            <defs>
              <path id="badgeCircle" d="M100,100 m-80,0 a80,80 0 1,1 160,0 a80,80 0 1,1 -160,0" />
            </defs>
            <text fill="var(--orange)" fontSize="13" letterSpacing="6" fontFamily="Bebas Neue, sans-serif">
              <textPath href="#badgeCircle" startOffset="0">
                ОБЪЕКТ ПОД КЛЮЧ ЗА 72 ЧАСА · ОБЪЕКТ ПОД КЛЮЧ ЗА 72 ЧАСА ·
              </textPath>
            </text>
          </svg>
        </div>
        <div className="w-16 h-16 grid place-items-center border border-[var(--orange)] text-gold">
